...
fn run_wasm_in_separate_process(addr: &str) -> io::Result<String> {
    ...
    let path = format!("/home/arjun/Desktop/mini-project/backend/server/src/savedWasmFunctions/{}", addr);
...
pub async fn run_wasm_function_direct(addr: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let module = Module::from_file(
        &engine,
        format!("/home/arjun/Desktop/mini-project/backend/server/src/savedWasmFunctions/{}", addr),
    )?;
